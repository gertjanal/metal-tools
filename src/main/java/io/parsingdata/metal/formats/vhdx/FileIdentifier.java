...
import static io.parsingdata.metal.formats.vhdx.Constants.UTF_16;
...
/**
 * Metal tokens for VHDX File Identifier.
...
public class FileIdentifier {
    public static final Token FILE_IDENTIFIER = seq("fileIdentifier",
        ...
        // This field may not be null terminated. This field is optional for the parser to fill in during the
        // creation of the VHDX file to identify, uniquely, the creator of the VHDX file.
        // Parsers must not use this field as a mechanism to influence parser behavior; it only exists for diagnostic purposes.
        // A parser must write File Type Identifier structure when the file is created and must validate the
        // Signature field when loading a VHDX file. The parser must not overwrite any data in the first 64 KB of
    ...
    /**
     * Create a cho for every possibility with def(size) + def(total - size) to get a null terminated String.
     * For example 'title\u0000\u0000\u0000', a def("title") and def("nod") will be returned.
     * @param name Name to set on the String token.
